...
for (y in 1:Iter)			#the entire simulation is repeated "Iter" number of times
   {
   for(x in seq(2,MS,1))		#the core of the simulation; each step is repeated for MS times (x=1 is the starting condition)
	{
	vector1	= 1:TT
  	...
  	while	(!(((length(index) == 0) || (all(MM[(x-1),index] < NINHIBIT[rn,index]))) && (all(MM[(x-1),] >= -NOUT[rn,]))))
		...
		rn <- sample(vector1, 1)	# this way, you're not going to pick a transition you've already discharged!
